...
//    [self.contentView addSubview:self.lineViewDown];
    ViewRadius(self.contentView, 15);
}
...
- (void)show
{
    ...
    if (self.contentMode == STPickerContentModeBottom) {
        ...
        frameContent.origin.y = ksHeight - 350;
        ...
        frameContent.origin.y = ksHeight - 285;
        [UIView animateWithDuration:0.3 delay:0 options:UIViewAnimationOptionCurveEaseOut animations:^{
            [self.layer setOpacity:1.0];
            self.contentView.frame = frameContent;
...
- (UIButton *)buttonRight
{
    if (!_buttonRight) {
        ...
        [_buttonRight setTitleColor:appMainColor forState:UIControlStateNormal];
