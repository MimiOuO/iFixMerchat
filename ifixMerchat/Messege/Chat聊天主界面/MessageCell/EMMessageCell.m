...
- (void)_setupViewsWithType:(EMMessageType)aType
{
    ...
    ViewRadius(_avatarView, 20);
    [self.contentView addSubview:_avatarView];
    if (self.direction == EMMessageDirectionSend) {
//        _avatarView.image = [UIImage imageNamed:@"user_avatar_me"];
        [_avatarView sd_setImageWithURL:currentUserAvatar];
...
- (EMMessageBubbleView *)_getBubbleViewWithType:(EMMessageType)aType
{
    ...
    switch (aType) {
        ...
        case EMMessageTypeFile:
            bubbleView = [[MioRedPacketBubbleView alloc] initWithDirection:self.direction type:aType];
            break;
...
- (void)setModel:(EMMessageModel *)model
{
    ...
    if (model.direction == EMMessageDirectionSend) {
        ...
        self.nameLabel.text = [model.emModel.ext objectForKey:@"nickname"];
        //头像
//        [self.avatarView sd_setImageWithURL:Url([[DuoduoDB getObjectById:model.emModel.from fromTable:userTable] objectForKey:@"avatar"])];
